...
- `record` (default true): per-session [asciinema](https://asciinema.org) cast recordings, on by default so watch and replay work out of the box. `record_dir` (default `recordings`) stores one `<session-id>.cast` per connection, capturing exactly the bytes the attacker saw; each cast is capped and the directory has file/byte quotas, so the default cannot fill the disk. The portal can replay them inline. Set `false` to disable.
