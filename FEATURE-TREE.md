...
- **WordPress admin "gives" after persistent credential-stuffing**: wp-login rejects a one-shot bot, but a source that keeps trying is let in (its guessed credential captured and logged as brute-forced), handed a logged-in cookie, and landed in a wp-admin dashboard that reveals an embedded payload, so persistent attackers earn a payoff and a louder log trail instead of an endless 302. _internal/proto/http: TestWPGateLetsInPersistentGuesser, TestWPLoginRevealsOnlyAfterWork, TestJTArtEmbedded_
